...
const ViewBookDetails = () => {
  ...
  const handleFavourite = async () => {
    const response = await axios.put(
      "http://localhost:1000/api/v1/fav",
      {},
      { headers: { ...headers, bookid: Data._id } }
    );
  ...
  const handleDeleteBook = async () => {
    if (window.confirm("Are you sure you want to delete this book?")) {
      try {
        const response = await axios.delete(`http://localhost:1000/api/v1/delete-book/${id}`, {
          headers,
        });
        alert(response.data.message);
      ...
      {Data ? (
        <div className="px-4 md:px-12 py-8 bg-zinc-900 flex flex-col md:flex-row gap-8">
          <div className="w-full lg:w-3/6">
            <div className="bg-zinc-800 rounded px-4 py-12 h-[60vh] lg:h-[88vh] flex justify-around">
